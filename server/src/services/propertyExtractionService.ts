import { PropertyOwnersProductData, PropertyOwnersSubmissionEnvelope } from "../schemas/products/propertyOwners.schema.js";
...
function findLine(text: string, pattern: RegExp): string | undefined {
  return text.split("\n").find((line) => pattern.test(line));
}

function evidence(field: string, source?: string, confidence = 0.9): Evidence {
  return { field, evidence: source ? compactSpaces(source) : undefined, confidence: source ? confidence : 0 };
}

function matchValue(text: string, regex: RegExp): { value?: string; evidence?: string } {
...
function parseBroker(text: string) {
...
function parseInsured(text: string) {
...
function parseLocations(text: string) {
  ...
  function splitOccupancyNotes(value: string): { occupancy: string; notes?: string } {
...
function parseLossHistory(text: string) {
...
function parseUnderwriting(text: string) {
...
function missingFields(submission: PropertyOwnersProductData): string[] {
...
export async function extractPropertySubmissionFromText(sourceFile: string, text: string): Promise<PropertyOwnersSubmissionEnvelope> {
  ...
  const draft = {
    product: "Property Owners Package" as const,
  ...
  return {
    sourceFile,
    productType: "property_owners" as const,
    common: {
      insured: {
